...
Token *tokenize_file(const char *filepath, int *out_count) {
    if(!filepath || !out_count) { if(out_count)*out_count=0; return NULL; }
    ...
    Token *toks = tokenize(buf, out_count);
    free(buf);
    return toks;
}

void tok_get_position(const Token *t, int *out_line, int *out_col) {
    if(!t){ if(out_line)*out_line=0; if(out_col)*out_col=0; return; }
    if(out_line) *out_line = t->line;
    if(out_col)  *out_col  = t->col;
}

void tok_to_str(const Token *t, char *out, int sz) {
    if(!t){ snprintf(out,sz,"Token(NULL)"); return; }
    snprintf(out,sz,"Token(%s, '%s', %d:%d)",
...
char lexer_peek_char(Lexer *l, int offset) {
    return lex_peek(l, offset);
}
...
void lexer_skip_whitespace(Lexer *l) {
    while(l->pos < l->src_len) {
        char c = lex_cur(l);
        if(c == ' ' || c == '\t' || c == '\r') lex_adv(l);
        else break;
    }
}

void lexer_skip_comment(Lexer *l) { /* SKIP??? WHAT??? * Bro just exes this */
    while(l->pos < l->src_len && lex_cur(l) != '\n')
        lex_adv(l);
}
...
char *lexer_read_identifier(Lexer *l) {
    int start = l->pos;
    while(l->pos < l->src_len &&
          (isalnum((unsigned char)lex_cur(l)) || lex_cur(l) == '_'))
        lex_adv(l);
    int len = l->pos - start;
    char *s = malloc((size_t)len + 1);
    if(s) { memcpy(s, l->src + start, (size_t)len); s[len] = 0; }
    return s;
}
...
char *lexer_read_number(Lexer *l) {
    ...
    if(lex_cur(l) == '0' && (lex_peek(l,1) == 'x' || lex_peek(l,1) == 'X')) {
        lex_adv(l); lex_adv(l);
        while(l->pos < l->src_len && isxdigit((unsigned char)lex_cur(l))) lex_adv(l);
    }
    else if(lex_cur(l) == '0' && (lex_peek(l,1) == 'b' || lex_peek(l,1) == 'B')) {
        lex_adv(l); lex_adv(l);
        while(l->pos < l->src_len && (lex_cur(l) == '0' || lex_cur(l) == '1')) lex_adv(l);
    }
    else {
        while(l->pos < l->src_len && isdigit((unsigned char)lex_cur(l))) lex_adv(l);
        if(lex_cur(l) == '.' && isdigit((unsigned char)lex_peek(l,1))) {
            lex_adv(l);
            while(l->pos < l->src_len && isdigit((unsigned char)lex_cur(l))) lex_adv(l);
        }
...
char *lexer_read_string(Lexer *l, char quote) {
    if(lex_cur(l) == quote) lex_adv(l);
    char buf[65536]; int bi = 0;
    while(l->pos < l->src_len && lex_cur(l) != quote && bi < (int)sizeof(buf) - 4) {
        if(lex_cur(l) == '\\') {
            lex_adv(l);
            char nc = lex_adv(l);
            switch(nc) {
    ...
    if(lex_cur(l) == quote) lex_adv(l);
    buf[bi] = 0;
    char *s = malloc((size_t)bi + 1);
    if(s) memcpy(s, buf, (size_t)bi + 1);
    return s;
}
