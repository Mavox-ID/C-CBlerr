...
class CCodeGenerator:
    ...
    def generate_for(self, for_stmt: ForLoop):
        ...
        if for_stmt.iter_var and for_stmt.iter_expr:
            ...
            self.emit_line("{")
            self.indent_level += 1
            self.emit_line("TODO: implement generic iteration") # TODO
            for stmt in for_stmt.body:
                self.generate_statement(stmt)
            self.indent_level -= 1
            self.emit_line("}")
...
class StandaloneCompiler:
    ...
    def _select_compiler(self, forced_compiler: Optional[str]) -> str:
        if forced_compiler:
            ...
            if self._compiler_exists(compiler_name):
                ...
                self.log(f"Error: compiler {forced_compiler} not found.", "Error")
                raise RuntimeError(f"Compiler {forced_compiler} not found.")
    ...
    def _handle_compile_error(self, error_output: str, debugger) -> bool:
        try:
            if "undefined reference" in error_output or "ld returned 1" in error_output or "undefined reference" in error_output:
                print(f"\n\033[31mLinker Error:\033[0m\n{error_output.strip()}", file=sys.stderr)
                return True

            m_implicit = re.search(r"(?:implicit declaration of function|implicit function declaration) ['‘]([^'’]+)['’]", error_output, flags=re.I)
            if m_implicit:
                ...
                if source:
                    ...
                    for line_num, line in enumerate(lines, 1):
                        if func_name in line:
                            exc = SyntaxError(f"Unknown function '{func_name}'")
                            exc.lineno = line_num
            ...
            m_ident = re.search(r"(?:error|error):\s*['‘]([^'’]+)['’]", error_output, flags=re.I)
            if m_ident and source:
                identifier = m_ident.group(1)
                lines = source.splitlines()
                for line_num, line in enumerate(lines, 1):
                    if identifier in line:
                        exc = SyntaxError(f"Unknown identifier: '{identifier}'")
                        exc.lineno = line_num
                        exc.offset = line.find(identifier) + 1
                        debugger.display_syntax_error(exc, source=source, filename=str(self.source_file))
                        return True
            
            m_msg = re.search(r"(?:error|error):\s*(.+?)(?:\n|$)", error_output, flags=re.I)
            if m_msg:
    ...
    def compile(self) -> bool:
        ...
        try:
            debugger.log_info("Starting Compile...")
            self.log(f"CBlerr Console Compiler")
            self.log(f"OS: {self.system}")
            self.log(f"Output file: {self.output_exe}")
            ...
            with open(self.source_file, 'r', encoding='utf-8') as f:
                source = f.read()
            self.log(f"  Readed {len(source)} bytes from {self.source_file}")
            ...
            try:
                ...
                if main_fn and not getattr(main_fn, 'is_extern', False):
                    ...
                    if found_return0 and not found_endofcode:
                        self.log('Use "endofcode" without return 0.')
            except Exception:
                import traceback
                traceback.print_exc()
                self.log("AST post-processing failed.", "Error")
                return False
            ...
            with open(self.c_file, 'w', encoding='utf-8') as f:
                f.write(c_code)
            self.log(f"  Generated {len(c_code)} bytes C-code.")
            self.log(f"  C-code saved to:{self.c_file}")

            self.log("\n[5/5] Compiling C-code to executable file...")
            success = self._compile_c_to_exe()
            if success:
                print("\033[92mCompilation Successfully!\033[0m")
            return success
    ...
    def _compile_msvc(self) -> bool:
        ...
        try:
            ...
            self.log(f"  Starting: cl.exe for code compile...")
            ...
            if result.returncode == 0 and exe_found:
                self.log(f"  Compiling via MSVC successful!")
                try:
                    keep = SAVE_C_FLAG or os.getenv('CBLERR_KEEP_C', '0') == '1'
                    if keep:
                        self.log(f"  Leaving the C file because of the -c flag.: {self.c_file}")
                    else:
                        if self.c_file.exists(): os.remove(self.c_file)
                        if self.obj_file.exists(): os.remove(self.obj_file)
                except Exception as e:
                    self.log(f"  Error delete: {e}", "Warn")
                return True
            else:
                combined = (result.stdout or "") + ("\n" if result.stdout and result.stderr else "") + (result.stderr or "")
                if not self._handle_compile_error(combined, self.debugger):
                    self.log(f"  Compile via MSVC unsuccessful!: {combined}", "Warn")
                return False
                
        except Exception as e:
            self.log(f"  Error MSVC: {e}", "Warn")
            return False
    ...
    def _compile_mingw(self) -> bool:
        ...
        try:
            ...
            self.log(f"  Starting: gcc.exe for code compile (Ultra mode)...")
            ...
            if result.returncode == 0 and exe_found:
                try:
                    ...
                    self.log(f"Size of the resulting file: {phys/1024.0:.2f} Kilobytes")

                    keep = SAVE_C_FLAG or os.getenv('CBLERR_KEEP_C', '0') == '1'
                    if keep:
                        self.log(f"  Leave a temporary C file: {self.c_file}")
                    else:
                        if self.c_file.exists(): os.remove(self.c_file)
                        if self.obj_file.exists(): os.remove(self.obj_file)
                except Exception as e:
                ...
                combined = (result.stdout or "") + ("\n" if result.stdout and result.stderr else "") + (result.stderr or "")
                if not self._handle_compile_error(combined, self.debugger):
                    self.log(f"  Compilation via MinGW failed!: {combined}", "Warn")
                return False
    ...
    def _compile_gcc(self) -> bool:
        ...
        try:
            ...
            self.log(f"  Running: gcc to compile the code...")
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=60)

            exe_found = self.output_exe.exists()

            if result.returncode == 0 and exe_found:
                self.log(f"  Compilation via GCC successful!")
                return True
            else:
                combined = (result.stdout or "") + ("\n" if result.stdout and result.stderr else "") + (result.stderr or "")
                if not self._handle_compile_error(combined, self.debugger):
                    self.log(f"  Error compile via GCC: {combined}", "Warn")
                return False
    ...
    def _compile_clang(self) -> bool:
        ...
        try:
            ...
            self.log(f"  Starting: {clang_cmd} for code compile...")
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=60)

            exe_found = self.output_exe.exists() or (self.is_windows and self.output_exe.with_suffix('.exe').exists())

            if result.returncode == 0 and exe_found:
                self.log(f"  Compile via Clang successful!")
                return True
            else:
                combined = (result.stdout or "") + ("\n" if result.stdout and result.stderr else "") + (result.stderr or "")
                if not self._handle_compile_error(combined, self.debugger):
                    self.log(f"  Compile via Clang unsuccessful: {combined}", "Warn")
                return False
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python build/build.py <source_file.cbl> [options]")
        print("Options:")
        print(" -o <file> Path to the output executable")
        print(" -t <target> Target platform: Windows (default) or Linux")
        print(" --verbose Show verbose output")
        print(" -c Save the generated C file")
        print(" -static Force static linking (passes -static to gcc)")
        print(" -dynamic Force dynamic linking (uses -shared-libgcc and implib)")
        print(" --stack-size <bytes|K|M> Reserve stack (in bytes or with K/M suffix)")
        print(" --gcc Use the GCC compiler")
        print(" --clang Use the Clang compiler")
        print(" --lld Use Clang + LLD linker")
        print(" --mingw Use MinGW (GCC for Windows)")
        sys.exit(1)
    ...
    while i < len(sys.argv):
        if sys.argv[i] == '-o' and i + 1 < len(sys.argv):
            ...
            try:
                ...
                print(f"Invalid stack size: {raw}")
                sys.exit(1)
            i += 2
        else:
            i += 1
    ...
    if success:
        print(f"Executable file: {compiler.output_exe}")
        sys.exit(0)
    else:
        print(f"\nCompile Error!")
        sys.exit(1)
